...
def printMachines(n,m1,m2,m3,maint):
	#prints a list of all of operations and maintenances and their times
	print ("machine one \t\t\t\t\t machine two \t\t\t\t\t machine three")

	for i in range(n):
		print ("nr.: {}  req: {} end: {} \t\t\t nr.: {} req: {} end: {} \t\t\t nr.: {} req: {} end: {}"
		.format(m1[i].id,m1[i].time,m1[i].endTime,m2[i].id,m2[i].time,m2[i].endTime,m3[i].id,m3[i].time,m3[i].endTime))

	#print("sum of times: machine one: {} \t machine two: {} \t machine three: {}".format(sumTimes(m1),sumTimes(m2),sumTimes(m3)))
	print("sums of end times : {} ".format(countEndTimes(m1,m2,m3)))
	#print("maintenances: ")
	#for j in range(len(maint)):
	#	print("*************************************************")
	#	print("{}-{}".format(maint[j].startTime,maint[j].endTime))

def load(n,m1,m2,m3,maint,instanceNumber):
	...
	for line in f:
		if count < n:
			...
			m3.append(Operation(count,int(number[2])))
			count+=1
		#else:
...
def setEndTimes(m1,m2,m3,maint):
	now1=0
	now2=0
	now3=0
	#bonus = 0
	for i in m1:
		...
		if isInBetweenInMaint(now1,math.ceil(i.time) + now1,maint):

			#bonus = 0
			#while isInBetweenInMaint(now1,math.ceil(i.time * (1-bonus)) + now1,maint):
			while isInBetweenInMaint(now1,math.ceil(i.time ) + now1,maint):
				now1+=1   
		#i.endTime = math.ceil(i.time * (1-bonus)) + now1
		i.endTime = math.ceil(i.time ) + now1
		now1 = i.endTime
		#if (bonus<0.25):
		#	bonus+=0.05
	for j in range(len(m2)):
		...
		now2 =m2[j].endTime
	for z in range(len(m3)):
		while now3 < getOpById(m2,m3[z].id).endTime:
			now3+=1
		m3[z].endTime = m3[z].time + now3
		now3 =m3[z].endTime

def countEndTimes(m1,m2,m3):
	total1=0
	total2=0
	total3=0
	li = list()
...
def ObjectiveFunction(m3):
	return m3[len(m3)-1].endTime
...
def setMachineTwo(m1,m2):
	...
	return new
def setMachineTHree(m2,m3):
	xa = list()
	#for op in m1:
	for op in m2:
		xa.append(getOpById(m3, op.id))
	return xa
...
def ant(m1,m2,m3,matr,inte,shortestMachine):
	done = list()
...
def ACO(m1,m2,m3,main,instanceNumber,randomOrderFunction):
	...
	for a in range (numberOfCycles):
		...
		for i in range(numberOfAnts):	
			paths.append(ant(m1,m2,m3,pher,interest,shortestMachine))
		...
		pathsThree.clear()
		for i in range(len(pathsTwo)):
			pathsThree.append(setMachineTHree(pathsTwo[i],m3))
			
		results.clear()
		for i in range(len(paths)):
			setEndTimes(paths[i],pathsTwo[i],pathsThree[i],main)
			# printMachines(len(m1),paths[i],pathsTwo[i],main)end
			##results.append([ObjectiveFunction(pathsTwo[i]),i])
			results.append([ObjectiveFunction(pathsThree[i]),i])
			results.sort()
	...
	p3 = setMachineTHree(p2,m3)
	setEndTimes(p1,p2,p3,main)
	printMachines(len(m1),p1,p2,p3,main)
	#print(" \t\t\tbest result:     {}".format(recordTime))
	#print()
	save(p1,p2,p3,main,instanceNumber,randomOrderFunction,recordTime)

	return recordTime

#out adı verilen dosya açıp son datayı içine kaydetme
def save(m1,m2,m3,maint,instanceNumber,randomOrderFunction,recordTime):
	...
	f.write("machine one \t\t\t\t\t\t\t\t\t machine two \t\t\t\t\t\t\t\t\t machine three\n")

	for i in range(len(m1)):
		f.write ("id: {}; time required: {}; op ended: {};\t\t\t id: {}; time required: {}; op ended: {}; \t\t\t id: {}; time required: {}; op ended: {}\n"
		.format(m1[i].id,m1[i].time,m1[i].endTime,m2[i].id,m2[i].time,m2[i].endTime,m3[i].id,m3[i].time,m3[i].endTime))
...
load(n,mOne,mTwo,mThree,main,instanceNumber)
setEndTimes(mOne,mTwo,mThree,main)
#randomOrderFunction = mTwo[len(mTwo)-1].endTime
randomOrderFunction = mThree[len(mThree)-1].endTime
printMachines(len(mOne),mOne,mTwo,mThree,main)
results = list()
for i in range(2):
	results.append(ACO(mOne,mTwo,mThree,main,instanceNumber,randomOrderFunction))
	print(results)
